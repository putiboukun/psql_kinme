...
3.kinmeで使用するDBをSQLiteからDuckDBへ変更<br>
...
することで環境を構築します。<br>
2つのコンテナが立ち上がったら　http://ホストip:8080/ (初期設定のままの場合)<br>
へアクセスすることで使用可能となります。<br>
なお、uploadできるworkflowの容量は50MBまでとなります。<br>
<br>
DuckDBのデータベースファイルは `.env` の `DUCKDB_DATABASE` でパスを指定できます。<br>
デフォルトではアプリケーションコンテナ内の `/var/www/app/data/kinme.duckdb` を使用します。<br>
<br>
<br>
メモ1<br>
...
<br>
メモ2<br>
DuckDBのデータベースファイルのパスやアクセスするポート番号は.envで規定されています。<br>
