...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'kinme')
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False

# Update the database URI to use PostgreSQL
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'postgresql://postgres:postgres@pskinme_db:5432/postgres')
...
class NewWorkflow(FlaskForm):
    ...
    knwf = FileField(u'knwf file', validators=[FileAllowed(['knwf'], 'knwf files only!')])
...
def edit_workflow(workflow_id):
    ...
    if form.validate_on_submit():
        ...
        return render_template("edit.html", workflow=wf, tag_str=tag_str, form=form)
    flash_errors(form)
    return render_template("edit.html", workflow=wf, form=form)
...
def delete_workflow(workflow_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect("/")
    return render_template("delete.html", workflow=wf, form=form)
